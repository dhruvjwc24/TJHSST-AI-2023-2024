...
def getSolution(h, w, state, file, ts):
    ...
    if not(h > 8 or w > 8): wordsDict = makeWordsDict(h, w, segments, state, file)
    else: wordsDict = makeWordsDict(h, w, segments, state, file, combined=True)
    if h > 8 or w > 8:
        wordsUsed = set()
        ...
        for segment in horizSegments:
            ...
            for word in wordsDict: 
                if word in wordsUsed or not(len(word) == segmentLen) or not(re.match(pattern, word)): continue
                wordsUsed.add(word)
