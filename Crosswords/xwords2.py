...
def getPatternAndFindWords2(segment, wordsDict, state, exclude=set(), justVerify=False):
...
def getFinalSolution(h, w, state, wordsDict, openSegments, graph, ts):
    global STATES_BY_SEGMENT, IDX_BY_SEGMENTS, PARTIALSTATE
...
def getFinalSolutionHelper(h, w, state, wordsDict, openSegments, graph, numVisited, wordsInBoard=set(), segmentsVisited = set(), ts=0):
    ...
    #CHANGED **** potWordsForParentSeg = getPatternAndFindWords(parentSeg, wordsDict, state, exclude=wordsInBoard, justVerify=False) 
    ...
    for potParentWord in potWordsForParentSeg:
        ...
        for childSeg in childSegs:
            #CHANGED ***** wordsFoundInChildSeg = getPatternAndFindWords(childSeg, wordsDict, potState, exclude=wordsInBoard | {potParentWord}, justVerify=True)
        ...
        if childrenWork:    ### IF THE PARENT WORD WORKED WITH CHILDREN AND RECUR
            partialStateCount = PARTIALSTATE.count('-')
            potStateCount = potState.count('-')
            if  partialStateCount > potStateCount:
                FINALSTATE = potState
...
def getSolution(h, w, state, file, ts):
    global FINALSTATE, PARTIALSTATE, pattern_finder
    wordsDict = {}
    ...
    if h > 8 or w > 8:
        ...
        for segment in segments:
            ...
            # pattern = getPattern(segment, state)
            potWordsForParentSeg = getPatternAndFindWords(segment, state, wordsUsed, justVerify=False)
            if potWordsForParentSeg:
                wordsUsed.add(potWordsForParentSeg[0])    
                state = updateState(state, segment, potWordsForParentSeg[0])
        # print2DState(state, h, w, True)
        FINALSTATE = state
        return state
...
def parseArgs(args):
    ...
    # if not args: args = "21x21 75 v18x7# h12x5".split()
    ...
    # if not args: args = "20x25 88 v2x6# H7x22 V13x6S".split()
    # if not args: args = "3x4 0".split()
    # if not args: args = "7x7 11 V0x2OLYMPIA H1x6# H1x1ALEXA".split()
    # if not args: args = "7x7 11 H3x4SSA V0x3DEL h6x0#".split()
...
def makeWordsDict(h, w, segments, baseState, file=None, combined=False):

    fileContent = ""
    # fileContent = open(args[0]).read().splitlines()
    # fileContent = open(file).read().splitlines()
    # if combined: return fileContent
    ...
    for segment in segments:
        ...
        distinctLengths.add(len(segment))
    for word in fileContent:
        word = word.strip().upper()
        wordLen = len(word)
        if wordLen < 3 or wordLen > maxDim or wordLen not in distinctLengths: continue
        for segment in segments:
