...
import re
#import psutil
#from TrieDict import Trie  # Import the class
class TrieNode:
    def __init__(self):
        self.children = {}
...
class Trie:
    ...
    def build_from_file(self, fileContent):
...
def getFinalSolution(h, w, state, wordsDict, openSegments, graph):
    ...
    ### FOLLOWING
    # openSegments = [(1, 5, 9, 13, 17), 
    #                 (2, 6, 10, 14, 18), 
    #                 (8, 9, 10, 11), 
    #                 (12, 13, 14, 15), 
    #                 (4, 5, 6, 7), (3, 7, 11, 15), (4, 8, 12, 16), (1, 2, 3), (16, 17, 18)]
...
def getSolution(h, w, state, file):
    ...
    if h > 8 or w > 8:
        ...
        for segment in horizSegments:
            ...
            state = updateState(state, segment, wordsDict[key].pop())
        # print2DState(state, h, w, True)
        FINALSTATE = state
        return state
    
    pattern_finder = PatternFinder()
    pattern_finder.build_trie_from_file(fileContent)  # Populate the trie with words from the file
...
def parseArgs(args):
    if not args: args = "15x15 37 V11x12# V7x2# V2x4T v2x3E h11x4A".split()
...
def makeWordsDict(h, w, file=None):
    ...
    print(f"Words in dictionary: {count}")
    return wordsDict, fileContent
