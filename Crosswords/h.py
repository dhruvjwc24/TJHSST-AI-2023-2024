...
def getAllValidChoices(h, w, state, words, openSegments, sort=False):
    # horizOpens, vertOpens = getChoices(h, w, state)
    ...
    for choice in openSegments:
        ...
        CHOICES_CACHE[choice] = wordsToFit[:50]
    ...
    #                 filters[relPlace] = state[-1*idx]
    #     wordsToFit = filterWords(words, openLen, filters)
    #     vertWordChoices[choice] = wordsToFit
    # horizWordChoices = {}
    # for choice in horizOpens:
    #     filters = {}
    #     openLen = len(choice)
    #     for relPlace, idx in enumerate(choice):
    #         if idx < 0:
    ...
    #                 filters[relPlace] = state[-1*idx]
    #     wordsToFit = filterWords(words, openLen, filters)
    #     horizWordChoices[choice] = wordsToFit
    # validsDict = vertWordChoices | horizWordChoices
...
def parseArgs(args):
    ...
    # if not args: args = "6x6 10 v0x1# v0x5 v1x1#".split()
