...
def getFinalSolution(h, w, state, wordsDict, openSegments, graph):
    ...
    # for segment in openSegments:
    sol = getFinalSolutionHelper(h, w, state, wordsDict, openSegments, graph, numVisited=0, wordsInBoard=wordsInBoard, segmentsVisited=set())
    return sol
...
def getFinalSolutionHelper0(h, w, state, wordsDict, openSegments, graph, numVisited, wordsInBoard=set()):
    ...
    for potParentWord in potWordsForParentSeg:
        ...
        if (result:=getFinalSolutionHelper0(h, w, potState, wordsDict, openSegments, graph, numVisited+1, wordsInBoard | {potParentWord})): return result
    return ""


def getFinalSolutionHelper1(h, w, state, wordsDict, openSegments, graph, numVisited, wordsInBoard=set(), segment_attempts=None):
    global FINALSTATE
    if segment_attempts is None:
        segment_attempts = {seg: 0 for seg in openSegments}  # Initialize attempts for each segment
    ...
    # Check if we've exceeded attempts for the current segment
    if segment_attempts[parentSeg] >= 100000:
        # Skip this segment for now; try the next one
        return getFinalSolutionHelper1(h, w, state, wordsDict, openSegments, graph, (numVisited + 1) % len(openSegments), wordsInBoard, segment_attempts)
    ...
    for potParentWord in potWordsForParentSeg:
        segment_attempts[parentSeg] += 1  # Increment attempt count for this segment
        ...
        if childrenWork:
            # Attempt to solve with the next segment
            result = getFinalSolutionHelper1(h, w, potState, wordsDict, openSegments, graph, numVisited + 1, wordsInBoard | {potParentWord}, segment_attempts)
            if result:
                return result
...
def getFinalSolutionHelper(h, w, state, wordsDict, openSegments, graph, numVisited, wordsInBoard=set(), segmentsVisited = set()):
    global FINALSTATE  # 
    ...
    for potParentWord in potWordsForParentSeg:
        ...
        if childrenWork:    ### IF THE PARENT WORD WORKED WITH CHILDREN AND RECUR
            ...
            result = getFinalSolutionHelper(h, w, potState, wordsDict, openSegments, graph, numVisited+1,  wordsInBoard | {potParentWord},segmentsVisited)
            if result:
                return result   
...
def getSolution(h, w, state, file):
    ...
    wordsDict = makeWordsDict(h, w, segments, file)
...
def makeWordsDict(h, w, segments, file=None):
    fileContent = open(args[0]).readlines()
