...
def updateState(state, open, wordToFit):
    if not wordToFit: return state
    stateLst = list(state)
    for relIdx, idx in enumerate(open):
        if idx < 0: continue
        stateLst[idx] = wordToFit[relIdx]
    return "".join(stateLst)

def getSolution(h, w, numBlockingSquares, state, words, horizOpens, vertOpens):
    wordsUsed = set()
    for open in horizOpens:
        filters = {}
        openLen = len(open)
        for relPlace, idx in enumerate(open):
            if idx < 0:
                if idx == -h*w:
                    filters[0] = state[0]
                else:
        ...
        state = updateState(state, open, wordToFit)
    return state


def parseArgs(args):
    if not args: args = "dict3.txt 13x13 27 H6x4no#on v5x5nor v0x0stride h0x4trip H0x9fall V0x12limp".split()
    ...
    h,w,numBlockingSquares,blocks,file = 3, 3, 0, [], "dict.txt"
    for arg in args:
...
def main():
    ...
    # args = "dict.txt 13x13 27 H6x4no#on v5x5nor v0x0stride h0x4trip H0x9fall V0x12limp".split()
    # with open(args[0]) as file:
    #     words = file.read().split()
    # if args: words = open(args[0]).read().split()
    h, w, numBlockingSquares, blocks, file = parseArgs(args)
    if not args: words = open(file).read().split()
    base = getStructure(h, w, numBlockingSquares, blocks)
    print2DState(base, h, w)
    print("\n\n")
    horizOpens, vertOpens = getOpens(h, w, base)
    sol = getSolution(h, w, numBlockingSquares, base, words, horizOpens, vertOpens)
    # print(base)
