...
def getFinalSolutionHelper2(h, w, state, wordsDict, openSegments, graph, numVisited, wordsInBoard=set()):
    if numVisited == len(openSegments):
        complete = True
        return True

    segment = openSegments[numVisited]
    pattern = getPattern(segment, wordsDict, state)

    visited = list()
    wordlen = len(segment)

    if pattern[1] == ".": wordkey = "-"+str(wordlen)
    elif pattern[1].lower()+str(wordlen) not in wordsDict.keys(): wordkey = "-"+str(wordlen)
    else: wordkey = pattern[1].lower()+str(wordlen)
    
    for word in wordsDict[wordkey]:
        if word.upper() in wordsInBoard or not re.match(pattern,word.upper()): continue
        visited.append(word)

        potState = state
        potState = updateState(potState, segment, word)
        failedcon = False
        childSegs = graph[segment]
        for con in childSegs:
            patcon = getPattern(con, wordsDict, potState)
            lencon = len(con)

            if patcon[1] == ".": patkey = "-"+str(lencon)
            elif patcon[1].lower()+str(lencon) not in wordsDict.keys(): patkey = "-"+str(lencon)
            else: patkey = patcon[1].lower()+str(lencon)

            matchlist = [word for word in wordsDict[patkey] if re.match(patcon,word.upper())]
            if matchlist == []:
                failedcon = True
                break

        if failedcon: potState = state; continue
        
        wordsInBoard.add(word.upper())
        match = getFinalSolutionHelper(h, w, potState, wordsDict, openSegments, graph, numVisited+1, wordsInBoard)
        if match:
            return match
        wordsInBoard.remove(word.upper())
        state = potState
