...
def getSegments(h, w, state, sort=False):
    ...
    sortedSegments = sorted(allSegments, key=lambda x: len(x))
...
def getFinalSolution(h, w, state, wordsDict, openSegments, graph, ts):
    global STATES_BY_SEGMENT, IDX_BY_SEGMENTS

    for idx, segment in enumerate(openSegments):
        IDX_BY_SEGMENTS[segment] = idx
        STATES_BY_SEGMENT[idx] = ''
...
def getFinalSolutionHelper(h, w, state, wordsDict, openSegments, graph, numVisited, wordsInBoard=set(), segmentsVisited = set(), ts=0):
    global FINALSTATE, PARTIALSTATE, NUM_RECURSIONS, STATES_BY_SEGMENT 
    ...
    for potParentWord in potWordsForParentSeg:
        ...
        if h < 19 and w < 19 and time.time() - ts > 29.75:
            FINALSTATE = state
            return True

        if h >= 19 and w >= 19 and time.time() - ts > 44.75:
            FINALSTATE = state
            return True
        ...
        if childrenWork:    ### IF THE PARENT WORD WORKED WITH CHILDREN AND RECUR
            STATES_BY_SEGMENT[IDX_BY_SEGMENTS[parentSeg]] = potState
...
def fillOneWay(segments, wordsDict, state, wordsUsed=set()):
    for segment in segments:
        word = wordsDict[segment].pop()
        pattern = getPattern(segment, state)
        if "." not in pattern: continue
        while word in wordsUsed and not(re.match(pattern, word)): word = wordsDict[segment].pop()
        wordsUsed.add(word)
...
def getSolution(h, w, state, file, ts):
    ...
    segmentsCopy = segments.copy()
    vertSegments = [segment for segment in segments if segment[0]+1 != segment[1]]
    horizSegments = [segment for segment in segments if segment[0]+1 == segment[1]]
    wordsDict = makeWordsDict(h, w, segments, state, file)
    wordsDictCopy = wordsDict.copy()
    # if h > 8 or w > 8:
    #     wordsUsed = set()
    #     horizSegments = [segment for segment in segments if segment[0]+1 == segment[1]]
    ...
    if "-" not in FINALSTATE:
        print2DState(FINALSTATE, h, w, True)
    else:
        minState = ""
        numDashes = 10000000
        for segment in STATES_BY_SEGMENT.keys():
            if STATES_BY_SEGMENT[segment] and STATES_BY_SEGMENT[segment].count("-") < numDashes:
                minState = STATES_BY_SEGMENT[segment]
                numDashes = STATES_BY_SEGMENT[segment].count("-")
        print2DState(minState, h, w, True)
        numHorizFilled, numVertFilled = 0, 0
        wordsUsed = set()
        for vertSeg in vertSegments:
            segmentStr = "".join([state[idx] for idx in vertSeg])
            if OPENCHAR not in segmentStr: 
                numVertFilled += 1
                wordsUsed.add(segmentStr)
        for horizSeg in horizSegments:
            segmentStr = "".join([state[idx] for idx in horizSeg])
            if OPENCHAR not in segmentStr: 
                numHorizFilled += 1
                wordsUsed.add(segmentStr)
        
        if numHorizFilled < numVertFilled:
            updatedState = fillOneWay(vertSegments, wordsDictCopy, minState, wordsUsed)
        else:
            updatedState = fillOneWay(horizSegments, wordsDictCopy, minState, wordsUsed)
        print2DState(updatedState, h, w, True)
    print(f"Total Time Taken: {time.time() - ts} seconds")


        
        # print2DState(minState, h, w, True)
        # for i in range(len(graph)-1, -1, -1):
        #     if STATES_BY_SEGMENT[i]:
        #         print2DState(STATES_BY_SEGMENT[i], h, w, True)
        #         break
...
def parseArgs(args):
    # if not args: args = "15x15 37 H6x2 h12x3# v7x14T v12x6T".split()
