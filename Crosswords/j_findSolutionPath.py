...
def getSolutionPathHelper(graph, state, segmentsPath, wordsUsed=set()):
    ...
    for relPlace, idx in enumerate(currSegment):
        if state[idx] != OPENCHAR: filters[relPlace] = state[idx]
    possibleWords = filterWords(CHOICES_CACHE[currSegment], len(currSegment), filters, wordsUsed)
    if not possibleWords: return ""
    for word in possibleWords:
        wordsUsed.add(word)
        newState = updateState(state, currSegment, word)
        # print2DState(newState, 3, 5); print()
        remainingSegmentsPath = segmentsPath[1:]
