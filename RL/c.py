...
def BFS(grf):
    ...
    for vertex in range(len(shortestPaths)):
        ...
        if grf["grfProperties"]["rwdValuation"] == 0:
            for nbr in nbrs:
                if shortestPaths[nbr][0]+1 != pathLength: continue
            ...
            maxRwdCanReach = 0
            for nbr in nbrs:
                if shortestPaths[nbr][1] > maxRwdCanReach:
                    maxRwdCanReach = shortestPaths[nbr][1]
                    setOfBestNbrs = {nbr}
                elif shortestPaths[nbr][1] == maxRwdCanReach:
                    setOfBestNbrs.add(nbr)
                else:
                    continue
            for nbr in setOfBestNbrs:
                if nbr == vertex-grf["grfProperties"]["width"]:
                    grfPath += "U"
                elif nbr == vertex-1:
                    grfPath += "L"
