...
def makePossibles(pzl):
    ...
    for idx, c in enumerate(pzl):
        if c == ".":
            ...
            possibleValues = "".join(lenData["symset"] - values)
            if len(possibleValues) == 1: 
                pzl = pzl[:idx] + possibleValues + pzl[idx+1:]
            possibles[idx] = (possibleValues)
...
def findBestPeriod(possibles):
    ...
    for key in possibles:
        if len(possibles[key]) == 1: return key
        if len(possibles[key]) < minLen:
            minLen = len(possibles[key])
            bestIdx = key
    return bestIdx
...
def bruteForce(pzl, myPossibles):
    ...
    if len(psbls) == 1: 
        subPzl = pzl[:bestDot] + psbls + pzl[bestDot+1:]
        del myPossibles[bestDot]
        for idx in lenData["idxChecks"][bestDot]: 
            if not idx in myPossibles: continue
            if psbls in myPossibles[idx]:
                myPossibles[idx] = myPossibles[idx].replace(psbls, "")
        return bruteForce(subPzl, myPossibles)

    # if myPossibles[bestDot] == None: return ""
    # updateStats("lstOfChoices"+ str(len(possibleSyms)))
    for choice in psbls:
        ...
        for idx in lenData["idxChecks"][bestDot]: 
            if not idx in myPossibles: continue
            if choice in myPossibles[idx]:
                changes.add(idx)
                myPossibles[idx] = myPossibles[idx].replace(choice, "")
                if(len(myPossibles[idx])==0): 
                    return None

        bf = bruteForce(subPzl, myPossibles)
        if bf: return bf
        myPossibles[bestDot] = remove
        for idx in changes: 
            if idx in myPossibles:
                myPossibles[idx] += choice
            else: myPossibles[idx] = choice
    print("No solution")
    return ""
