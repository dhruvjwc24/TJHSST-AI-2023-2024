    ...
    for idx, c in enumerate(pzl):
        if c == ".":
            ...
            if len(possibleValues) == 1: 
                pzl = pzl[:idx] + possibleValues.pop() + pzl[idx+1:]
                possibleValues = None
...
def bruteForce(pzl, myPossibles):
    ...
    pzl, myPossibles = fwdLooking(pzl, myPossibles)
    if myPossibles == "": return None
...
def main():
    ...
    for idx, pzl in enumerate(PZLS):
        ...
        # for idx, c in enumerate(pzl):
        #     if c in lenData["idxChecks"][idx]: print(f"{idx:2} {c} {lenData['idxChecks'][idx]}")
