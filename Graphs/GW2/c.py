...
def BFS(grf):
    ...
    for vertex in range(len(shortestPaths)):
        for rwdIdx in rwdIdxs:
            ...
            path = shortestPath(vertex, rwdIdx, grf)
    ...
    for vertex in range(len(shortestPaths)):
        ...
        for edge in edges:
            if shortestPaths[edge]+1 == pathLength:
    ...
    return grfPaths, jumpsUsed
            ...
            # print(path, shortestPaths[vertex])
            # if pathLength > 0:
            #     return path, pathLength

def shortestPath(root, goal, grf):
    parseMe = [root]
    dctSeen = {root: ""}
    while parseMe:
        item = parseMe.pop(0)
...
def findPath(vertex, grf):
    if grf["edges"][vertex][1]: return (vertex, 0)
    parseMe = [vertex]
    dctSeen = {vertex: ""} #list of parents visited
    # s1 = 0
    while parseMe:
        node = parseMe.pop(0)
        # s2 = 0
        for nbr in grf["edges"][node][0]:
            # print(f"{s1}.{s2}")
            # s2+=1
            if grf["edges"][nbr[0]][1]:
                path = (dctSeen[node] + " " + node + " " + nbr).strip()
                pathLength = len(path.split(" "))-1
                return(path, pathLength)
                
            if nbr[0] not in dctSeen:
                dctSeen[nbr[0]] = f"{dctSeen[node]} {node}"
                parseMe.append(nbr[0])
        # s1+=1
    path = (dctSeen[node] + " " + node + " " + nbr).strip()
    pathLength = len(path.split(" "))-1
...
def main():
    ...
    # args = "GG20W10 V2,8R E@16,4,5~8,7,4R0 E@4,3,1N~R1 E*12,8,13S~R"
    # args = "G6 V-1R23 E0=5"
    # args = "G15W3 E0::3=2::3 V6R37"
