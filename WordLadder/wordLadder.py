...
def round_it(x, sig): return round(x, sig-int(floor(log10(abs(x))))-1)
...
def word_count(): return len(words)

def get_max_deg(): return max([graph[word][1] for word in graph])
...
def degree_list(): return [degree_count(deg) for deg in range(get_max_deg()+1)]

def degree_count(deg): return sum([graph[word][1]==deg for word in graph])

def edge_count(): return sum([graph[word][1] for word in graph])//2

def check_edge(w1, w2): return (len(w1)==len(w2) and (sum([c1==c2 for c1, c2 in zip(w1, w2)])==len(w1)-1))
...
def main():
    global args, words, graph, components
    start = time.process_time()
    if len(args) == 0: args = ["WordLadder/inputShort.txt", "caring", "dining"]
    words = open(args[0]).read().splitlines()
    graph = create_graph()
