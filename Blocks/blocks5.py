...
def bruteForce(state, blocksChoices, choicesMade=None):
    ...
    for choice in blocksChoices:
        ...
        if updatedState: 
            ...
            if bf != "No decomposition": return bf, choices
        elif not updatedState and choice[0] != choice[1]: 
            updatedState = updateState(choice[1], choice[0], (emptyPos // blockDims[1], emptyPos % blockDims[1]), state)
            if updatedState: 
                blockChoicesCopy = blocksChoices.copy()
                blockChoicesCopy.remove(choice)

                bf, choices = bruteForce(updatedState, blockChoicesCopy, choicesMade + [(choice[1], choice[0])])
                if bf != "No decomposition": return bf, choices
...
def main():
    ...
    blocks += [(1, 1) for _ in range(num1x1s)] 
