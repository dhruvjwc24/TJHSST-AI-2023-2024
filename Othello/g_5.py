...
def getNumSafeEdges(brd, token):
    ...
    for side in walls:
        for pos in walls[side]:
            if brd[pos] == "token" and checkSafeEdge(pos, walls[side], brd, token): safeEdges += 1
    return safeEdges
...
def midgameAlphaBeta(brd=None, tkn=None, alpha=-1000, beta=1000, level=2):
    ...
    for mv in possibleMoves:
        ...
        bsf = [score] + nm[1:] + [mv]
        if level == 2: print(f'Negamax scor;e: {bsf[0]}; Move Sequence: {bsf[1:]}\tMidgameAB')
        alpha = score
    return bsf
