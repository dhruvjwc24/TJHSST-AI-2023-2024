...
def quickMove(brd, tkn):
    ...
    if brd.count(".") <= hLim: 
        # print("terminal")
        return terminalAlphaBeta(brd, tkn)[-1]
    # if brd.count(".") <= midLim: 
    #     # print("midgame")
    #     return midgameAlphaBeta(brd, tkn)[-1]
    # else: 
    #     # print("rot")
...
def ruleOfThumb2(board, token, possibles=None):
    ...
    # bestScore = ruleOfThumbBrdEval(newBrd, token)
    ...
    if corners.intersection(validMovesSet): return corners.intersection(validMovesSet).pop()

    for move in validMoves:
        if move in edges and checkSafeToken(move, board, token): return move
    for move in validMoves:
        if checkSafeToken(move, board, token): return move
    ...
    newBrd = makeMove(board, bestMove, validMoves, token)
    eTkn = getOppositeToken(token)
    bestMobility = len(findPossibleMoves(newBrd, eTkn))
    for move in validMovesSet:
        newBrd = makeMove(board, move, validMoves, token)
        mobility = len(findPossibleMoves(newBrd, eTkn))
        if mobility < bestMobility: bestMobility = mobility; bestMove = move
    return bestMove
...
# def brdEval2(board, token):
    ...
    evaluation += 100*getNumCorners(board, token)**4
    evaluation += 75*getNumSafeTokens(board, token)**3
    evaluation -= 50*len(findPossibleMoves(board, getOppositeToken(token)))**2
    evaluation += 25*board.count(token)**1

    # for idx, space in enumerate(board):
    #     if space == token:
    #         if idx in corners: evaluation+=100
    #         if checkSafeToken(idx, board, token): evaluation+=75
    #         evaluation-= len(findPossibleMoves(makeMove(board, idx, possibles, token), getOppositeToken(token)))
    return evaluation
    # if possibles: validMoves = possibles
    # else: validMoves = findPossibleMoves(board, token)
    # validMovesSet = set(validMoves.keys())
    # # weights = {"corner": }
    scores = {move: 0 for move in validMoves}
    if len(validMoves) == 0: return -1
    ...
    for move in validMoves: 
        ...
        #Check stability
        if checkSafeToken(move, board, token): scores[move] += 75
        #Check mobility
        scores[move] -= len(findPossibleMoves(makeMove(board, move, validMoves, token), getOppositeToken(token)))
        #Check numMoves
        scores[move] += len(validMoves[move])
...
def midgameAlphaBeta(brd=None, tkn=None, alpha=-1000, beta=1000, level=2):
    ...
    for mv in orderedMoves:
        ...
        bsf = [score] + nm[1:] + [mv]
        if level == 4: print(f'Negamax score: {bsf[0]}; Move Sequence: {bsf[1:]}\tMidgameAB')
        alpha = score
    return bsf

def terminalAlphaBeta(brd=None, tkn=None, alpha=-1000, beta=1000, level=0):
    eTkn = "o" if tkn == "x" else "x"
    ...
    for mv in orderedMoves:
        ...
        if score <= alpha: continue
        bsf = [score] + nm[1:] + [mv]
        if level == 0: print(f'Negamax score: {bsf[0]}; Move Sequence: {bsf[1:]}\tTerminalAB')
        alpha = score
    return bsf
...
def main():
    ...
    if validMoves:
        ...
        if board.count(".") <= HLLIM:
            ab = terminalAlphaBeta(board, token)
            print(f'Negamax score: {ab[0]}; Move Sequence: {ab[1:]}')
        # elif board.count(".") <= 20:
        #     # intially 56
        #     ab = midgameAlphaBeta(board, token)
        #     print(f'Negamax score: {ab[0]}; Move Sequence: {ab[1:]}')
