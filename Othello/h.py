...
def extractFromArgs(args):
    board, token, suppress, moves, holeLimit, verbose, noArgs = '', '', False, [], 11, False, False
...
def brdEval(board, token):
    ...
    #17
    evaluation -= len(findPossibleMoves(board, eTkn))
...
def midgameAlphaBeta(brd=None, tkn=None, alpha=-1000, beta=1000, level=2):
    ...
    for mv in possibleMoves:
        ...
        bsf = [score] + nm[1:] + [mv]
        if level == 2: print(f'Negamax score: {bsf[0]}; Move Sequence: {bsf[1:]}')
        alpha = score
    return bsf
...
def terminalAlphaBeta(brd=None, tkn=None, alpha=-1000, beta=1000, level=0):
    ...
    for mv in orderedMoves:
        ...
        if score <= alpha: continue
        bsf = [score] + nm[1:] + [mv]
        if level == 0: print(f'Negamax score: {bsf[0]}; Move Sequence: {bsf[1:]}')
        alpha = score
    return bsf

def checkGameOver(board): return board.count(".") == 0 or not(findPossibleMoves(board, 'x') or findPossibleMoves(board, 'o'))

# def getHardMove(brd, moves):
#     moveStr = str(moves[60-brd.count('.')])
#     # moveStr = str(move)
#     if "_" in moveStr: moveStr = moveStr[1]
#     move = int(moveStr)
#     return move

def main():
    global args, CACHEMOVES, HLLIM, bestMoves
    # if not args: args = "193441212920111813261012373317_32540_2_1_4_5322416_8_9_0-149485657504358-142-1_61415_74423225130".split()
    if not args: args = "37292130201123131243_538_219472231_3_4394653101418_9_726_6152517_1_034_8241632403341444245516152605950495662485463555857 v".split()
    ...
    # board, token, suppress, oMoves, holeLimit, verbose, noArgs = extractFromArgs("193441212920111813261012373317_32540_2_1_4_5322416_8_9_0-149485657504358-142-1_61415_74423225130".split())
    ...
    if validMoves:
        # if token == "o": print(f"My preferred move is: {getHardMove(board, oMoves)}")
        if not validMoves: print("No moves possible")
        elif board in bestMoves: print(f"My preferred move is: {bestMoves[board]}")
        else:
        # if board == '.'*27+'ox......xo'+'.'*27: return 44
