...
def findPossibleMovesEmptyFirst(board, token):
    ...
    return validMoves           
                                ...
                                # if board[posToIdx(currR+ri, currC+ci)] == token:
                                #     if idx in validMoves: validMoves[idx] |= {posToIdx(currR, currC)}
                                #     else: validMoves[idx] = {posToIdx(currR, currC)}
                                #     break
                                # elif board[posToIdx(currR+ri, currC+ci)] == eTkn: break
                                # currR += ri
                                # currC += ci
                    # if r+ri >= 0 and r+ri < 8 and c+ci >= 0 and c+ci < 8:
                    #     if re.search(f"^{oppositeToken}+{token}.*", getLine(board, r, c, ri, ci)):
...
def main():
    global args, CACHE
    if not args: args = "...xo..oxoxoxxo.xoox.xxxxo.oooxxxoxooxxxooooooxooooooxx.oooooox. x".split()
    board, token, suppress, moves = extractFromArgs(args)

    # print(findPossibleMovesEmptyFirst(board, token))
    # print(findPossibleMovesTokenFirst(board, token))
    # print(findPossibleMoves(board, token))
