...
def extractFromArgs(args):
    board, token, suppress, moves, holeLimit, verbose, noArgs = '', '', False, [], 10, False, False
...
def quickMove(brd, tkn):
    ...
    if brd.count(".") > 57: return ruleOfThumb(brd, tkn)
    elif brd.count(".") > HLLIM: return midgameAlphaBeta(brd, tkn)[-1]
    return terminalAlphaBeta(brd, tkn)[-1] 
    if brd.count(".") <= HLLIM: return terminalAlphaBeta(brd, tkn)[-1]
...
def brdEval(brd, tkn):
    weights = {"countCorners": 100}
    # possibleMoves = findPossibleMoves(brd, tkn)
    # if not possibleMoves: return brd.count(tkn) - brd.count(getOppositeToken(tkn))
    return weights["countCorners"]*countCorners(brd, tkn) + brd.count(tkn)

def midgameAlphaBeta(brd=None, tkn=None, alpha=-1000, beta=1000, level=1):
    eTkn = "o" if tkn == "x" else "x"
    possibleMoves = findPossibleMoves(brd, tkn)

    if not possibleMoves: 
        ePossibleMoves = findPossibleMoves(brd, eTkn)
        if not ePossibleMoves: return [(brd.count(tkn) - brd.count(eTkn))*100000]
...
def main():
    ...
    if not args: args = "ooooo.o.ooooxxx.ooooo...oooox...xxxoxx...xx.xx...x.xxxx......... 57".split()
