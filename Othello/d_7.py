...
def checkSafeEdge(pos, walls, brd, token):
    oppositeToken = getOppositeToken(token)
    for wall in walls:
        if pos in walls[wall]:
            line = walls[wall]
            ltStr = brd[:line.index(pos)]
            ltStrRel = ltStr[ltStr.rfind(token):]
            rbStr = brd[line.index(pos)+1:][::-1]
            rbStrRel = rbStr[rbStr.rfind(token):]
            break

    match = re.search(f"^{token}{oppositeToken}+$", ltStrRel) or re.search(f"^{token}{oppositeToken}+$", rbStrRel)
...
def main():
    ...
    while i < len(moves):
        ...
        #     continue
        # print(f"Best move for {token}: {quickMove(board, token)}")
